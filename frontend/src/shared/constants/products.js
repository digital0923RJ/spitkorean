...
export const PRODUCTS = {
  TALK: {
    ...
    icon: "MessageCircle",
    route: "/talk",
    features: ["AI 튜터와 실시간 대화", "감정 인식 및 피드백", "모국어 해설 지원", "레벨별 맞춤 대화"],
  ...
  DRAMA: {
    ...
    icon: "Film",
    route: "/drama",
    features: ["실제 드라마 문장 학습", "문법 피드백", "유사 문장 제시", "발음 평가"],
    levels: ["beginner", "intermediate", "advanced"],
    tag: "문장 구성",
  },
  ...
  TEST: {
    ...
    icon: "BookOpen",
    route: "/test",
    features: ["TOPIK 모의고사", "문제 자동 생성", "약점 분석", "실전 시험 시뮬레이션"],
  ...
  JOURNEY: {
    ...
    icon: "Map",
    route: "/journey",
    features: ["한글 기초부터 고급 리딩까지", "발음 정확도 분석", "속도 조절 연습", "단계별 리딩 콘텐츠"],
...
export const getProductsByPrice = (ascending = true) => {
  return [...PRODUCT_LIST].sort((a, b) => (ascending ? a.price - b.price : b.price - a.price))
}
...
// 두 가지 이름으로 동일한 데이터를 export (호환성을 위해)
export const BUNDLES = bundleData
export const BUNDLE_PACKAGES = {
  bundle_2: bundleData.BUNDLE_2,
  bundle_3: bundleData.BUNDLE_3,
  bundle_all: bundleData.BUNDLE_ALL,
}
